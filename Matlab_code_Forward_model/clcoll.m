 function P_cosmo = clcoll(coll,sample,Lambda_f,so_f,EL_f,EL_mu,Psi_Cl36_Ca_0,rho_rock,rho_coll,Param_LSD,flag)
...
nc = size(coll,2) ;
if nc ~= 62, error('Sample file must have 62 columns'), end
chimie = sample(1:n-4) ;
z = sample(n-3) ; % (g.cm-2)
...
%if(strcmp(flag.mu_model,'exp'))
Lambda_mu = flag.L_mu; % g.cm-2
%end
...
% (Elements are given directly in ppm) - HOST ROCK
ppm = chimie ;
...
ppmc(59) = sum([coll(44:53) coll(58)]) - sum([ppmc(44:53) ppmc(58)]) ; % O rock in percent
ppmc(60) = O_water ;
...
N_k = (ppm(:,1:61)./A_k)*Avogadro*1e-6 ; % Concentrations in atom/g ROCK
...
ndepths=10; % Depth integration factor
...
elseif(strcmp(flag.mu_model,'lsd'))
 
   P_sp = mean( (P_sp_Ca + P_sp_K + P_sp_Ti + P_sp_Fe).*exp(-depths./Lambda_f) );
...
Psi_mu_0 = 190 ; % 190 slow negative muon stopping rate at land surface (muon/g/an), Heisinger et al. (2002)

f_c_Ca = (Num_k(49)*ppm(62)*1e-6/A_k(49))/(sum(Num_k.*ppmc(:,1:61)./A_k)*1e-6) ; % for Ca (ICP)
f_c_K = (Num_k(51)*ppm(51)*1e-6/A_k(51))/(sum(Num_k.*ppmc(:,1:61)./A_k)*1e-6) ; % for K
...
elseif(strcmp(flag.mu_model,'lsd'))
    ...
    totalfluxdepth = .0;
    P_mu = .0;
   if(max(depths)<=max(Param_LSD.depthvector))
...
%first, interpolate and make sure we have the surface values
    negfluxdepth=mean(interpolate(Param_LSD.depthvector,Param_LSD.negflux,depths));
    totalfluxdepth=mean(interpolate(Param_LSD.depthvector,Param_LSD.totalflux,depths));
% Muon
    P_mu=mean(interpolate(Param_LSD.depthvector,Param_LSD.Prodmu(1,:),depths));
    end
end
...
Y_s = sum(f_d_k.*Y_n.*ppmc(:,1:61).*Num_k./A_k)/sum(ppmc(:,1:61).*Num_k./A_k) ;
...
elseif(strcmp(flag.mu_model,'lsd'))
    ...
    phi_eth_total = mean((phi_star_eth .* exp(-depths./Lambda_f) + ...
...
A_eth = phi_star_eth ; A_eth = A_eth*(f_eth/Lambda_fth)*(1 - p_E_th) ;
B_eth = (1 + R_mu*R_eth)*FDeltaphi_star_eth ; B_eth = B_eth*(f_eth/Lambda_fth)*(1 - p_E_th) ;
C_eth = R_mu*phi_star_eth ; C_eth = C_eth*(f_eth/Lambda_fth)*(1 - p_E_th) ;
...
FDeltaphi_star_eth_a = (D_eth*Deltaphi_star_eth_a/L_eth - D_eth*Deltaphi_2star_eth_a/Lambda_f)/ ...
...
JDeltaphi_star_th = (D_th_a*(phi_star_th_a/Lambda_f - JDeltaphi_star_eth_a/L_eth_a) - ...
...
elseif(strcmp(flag.mu_model,'lsd'))
    ...
    phi_th_total = mean(phi_star_th.*exp(-depths./Lambda_f) + ...
...
%X = (sum(ppmc(:,1:61).*S_i.*Y_U_n))/(sum(S_i.*ppmc(:,1:61))) ;
% X DEPENDS ON CHEMICAL COMPOSITION - COLLUVIUM
%
%Y = (sum(ppmc(:,1:61).*S_i.*Y_Th_n))/(sum(S_i.*ppmc(:,1:61))) ;
% Y DEPENDS ON CHEMICAL COMPOSITION - COLLUVIUM
%
%U = ppmc(37) ; % Concentration en Uranium (ppm)
%Th = ppmc(35) ; % Concentration en Thorium (ppm)
%
...
if(strcmp(flag.mu_model,'exp'))
    ...
    P_cosmo = so_f*EL_f*(Q_sp.*P_sp + S_L_th*Q_th*P_th + S_L_eth*Q_eth*P_eth) + so_mu*EL_mu*Q_mu.*P_mu ;
elseif(strcmp(flag.mu_model,'lsd'))
