...
mflux = Muons(h,Rc,s);%Generates muon flux at site from Sato et al. (2008) model
mfluxRef = Muons(1013.25,0,consts.SPhiInf); %Generates SLHL muon flux from Sato et al. (2008) model
...
%end;
    ...
    tol = phi_vert_slhl * 1e-4;
    func_integ = @(aj,tolj)integral(@(x)Rv0(x).*ppval(RzSpline,x),aj,(2e5+1),'RelTol',tolj);
