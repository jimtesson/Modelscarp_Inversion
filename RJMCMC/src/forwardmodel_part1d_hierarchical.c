...
/*    for (gi = 0; gi < s->nglobalparameters; gi++) {
        s->global_parameters[gi] =
        rjmcmc_random_choose_double(s->global_parameters[gi].fmin,
                                    s->global_parameters[gi].fmax,
                                    random);
            printf("test:%f",s->global_parameters[gi]);
			 ...
			 double proposed)
{
  ...
  if (s->accepted) {
    ...
    part1d_forwardmodel_write(s->proposed,s->current_like);
  }
