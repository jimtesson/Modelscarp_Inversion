		      ...
		      int iy)
{
  ...
  for (i = 1; i < p->npartitions; i ++) {
	...
	p->ind[j + 1] = p->ind[j];
      ...
      p->ind[i] = iy;
      p->npartitions ++;
        //printf("inserted: i:%d, pos(i):%f, ind(i):%d\n",i,x,iy);
      return 0;
    }
  }
...
/* new function to produce orderer sequence of ages*/

int
position_map1d_inext(position_map1d_t *p,
		      int iy)
{
  int i;
  int j;
...
//			   "position_map1d_insert: max partitions\n");
    ...
    //printf("ind[%d]:%d indNEXT[i-1]: %d\n",iy,p->ind[iy],p->ind[iy-1]);
      return p->ind[iy-1];
    
//        for (i = 0; i < p->npartitions; i ++) {
            
//        if(p->ind[i]==(p->ind[iy]-1)){
//                printf("ind[%d]:%d indNEXT[i+1]: %d\n",iy,p->ind[iy],p->ind[i]);/**/
//            return p->ind[i];
...
int
position_map1d_iprev(position_map1d_t *p,
                     int iy)
{
    int i;
...
int
position_map1d_give_index(position_map1d_t *p,
                            int i)
{
    int j;
...
int
position_map1d_give_index2(position_map1d_t *p,
                          int i)
{
    int j;
    int k;
    ...
    RJMCMC_NULLCHECKINT(p, "position_map1d_next_index: null map\n");
    for (j = 0; j < p->npartitions; j ++) {
        //printf("give2: i:%d ind[%d]: %d, pos[%d]: %f pos[in[%d]]: %f\n",i,j,p->ind[j],j,p->pos[j],p->ind[j],p->pos[j]);
        if(p->ind[j]==i){
            k=j;
        //printf("give2 found: i:%d ind[%d]: %d, pos[%d]: %f\n",i,j,p->ind[j],j,p->pos[j]);
