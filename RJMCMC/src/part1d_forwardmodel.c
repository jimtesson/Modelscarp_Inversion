...
void
part1d_forwardmodel_write(const part1d_forwardmodel_t *src,
                          double misfit)
{
    int i;
    int j;
...
/*
  ...
  dst->npartitions = src->npartitions;*/
    ...
    sprintf(str_tmp, "%d,", src->npartitions-1);
    strcat(str,str_tmp);
     /*Slip-rate and Quiescence period*/
  for (i = 0; i < src->nglobalparameters; i ++) {
      sprintf(str_tmp, " %.3f", src->global_parameter[i]);
      strcat(str,str_tmp);
  }
    sprintf(str_tmp, ",");
    strcat(str,str_tmp);
    /*Age of each event*/
  for (i = 0; i < src->npartitions-1; i ++) {
      /* age */
      j = position_map1d_give_index(src->p,i);
      age = src->models[j].local_parameter[0];
      sprintf(str_tmp, " %.0f", age);
      strcat(str,str_tmp);
  }
    sprintf(str_tmp, ",");
    strcat(str,str_tmp);
    ...
    for (i = 0; i < src->npartitions-1; i ++) {
        ...
        j = position_map1d_give_index(src->p,i);
        k =position_map1d_give_index(src->p,i+1);
        slip = position_map1d_position_of_index(src->p,k) - position_map1d_position_of_index(src->p,j);
        
        sprintf(str_tmp, " %f", slip);
        strcat(str,str_tmp);
    }
    ...
    sprintf(filename, "../results/%d_results.txt", mpirank);
			       ...
			       rjmcmc_normal_rand_t normal)
{
  ...
  for (pi = 2; pi < npart; pi ++) {
    ...
    /* generate an array of ages */
    tmp = rjmcmc_create_array_1d(npart);
    li = 0;
    for (i = 0; i < npart; i ++) {
        tmp[i]=	rjmcmc_random_choose_double(local_parameters[li].fmin,
                                            local_parameters[li].fmax,
                                            random);
    }
    /* sorting the array of ages */
      for (i = 0; i < (npart-1); i ++) {
          for (j = 0; j < (npart-1); j ++) {
              if(tmp[j]>tmp[j+1]){
                  tmpsort = tmp[j];
  ...
  for (i = 0; i < npart; i ++) {
	...
	rjmcmc_random_choose_double(local_parameters[li].fmin,
     ...
     /* new way of assigning ordered ages*/
        j = position_map1d_give_index(p->p,
                                 i); /* searching for the index */
        p->models[j].local_parameter[li] = tmp[i] ; /* assining with the good part */
    }
  }
			     ...
			     double *localparameters)
{
  ...
  case PART1D_FM_ZERO:
	...
	return -1;
      ...
      //printf("current->models[%d]: %f\n",iy,current->models[iy].local_parameter[0]);
				  ...
				  double *birth_prob)
{
  ...
  if (part1d_forwardmodel_value_at(current, 
    ...
    //printf("npart: %d, new_iy=%d, new_x: %f\n",npart,new_iy,new_x);
    j = position_map1d_give_index2(proposed->p,new_iy); /* searching for the index */
    //printf("new_iy:%d, j: %d\n",new_iy,j);
    if(j>npart-2){printf("************* pb birth j=%d\n",j);}
  ...
  for (li = 0; li < proposed->nlocalparameters; li ++) {
    ...
    proposed->models[new_iy].local_parameter[li] = proposed->local_scratch[li] + dv;
      //printf("new age: %f\n",proposed->models[new_iy].local_parameter[li]);
        
       //   for (l=0; l<npart; l++){
       //       printf("model i: %d, model: %f \n",l,proposed->models[l].local_parameter[li]);
              
       //   }
          // check the proposed model age has a chronology:
        //
          
          if(npart <3){ //if there is a single event
          ...
          else{ //if there is more than 1 event
              
              // oldest event case
              if(j == npart-2){
                  inext = position_map1d_inext(proposed->p,j);// index of the next event age (younger)
                  //printf("1- inext: %d, age of the next: %f \n",inext,proposed->models[inext].local_parameter[li]);
                  if(proposed->models[new_iy].local_parameter[li] > proposed->models[inext].local_parameter[li]){
                      //printf("birth ok\n");
                      itest = 1;
                      //return 0;
                  }
              ...
              // youngest event case
                  ...
                  iprev = position_map1d_iprev(proposed->p,j);// index of the previous event age (older)
                  //printf("2- iprev: %d, age of the prev: %f \n",iprev,proposed->models[iprev].local_parameter[li]);
                  if(proposed->models[new_iy].local_parameter[li] < proposed->models[iprev].local_parameter[li]){
                      //printf("birth ok\n");
                      itest = 1;
                      //return 0;
                  }
              ...
              // other events case
              else if(j>0 && j<(npart-2)){
                  inext = position_map1d_inext(proposed->p,j);// index of the next event age (younger)
                  iprev = position_map1d_iprev(proposed->p,j);// index of the previous event age (older)
                  //printf("3- inext: %d, age of the next: %f \n",inext,proposed->models[inext].local_parameter[li]);
                  //printf("3- iprev: %d, age of the prev: %f \n",iprev,proposed->models[iprev].local_parameter[li]);
                  if(proposed->models[new_iy].local_parameter[li] > proposed->models[inext].local_parameter[li]
                     && proposed->models[new_iy].local_parameter[li] < proposed->models[iprev].local_parameter[li]){
                      //printf("birth ok\n");
                      itest = 1;
                      //return 0;
                  }
					...
					int *li)
{
  ...
  part1d_forwardmodel_clone(current, proposed);
   ...
   // for (iy = 0; iy < proposed->npartitions; iy ++) {
       // printf("models %d: %f %f \n",iy,current->models[iy].local_parameter[0],proposed->models[iy].local_parameter[0]);
    //}
  ...
  if (current->type == PART1D_FM_ZERO) {
    ...
    while (itest==0 ) {
        ...
        j = position_map1d_give_index2(proposed->p,iy); /* searching for the index */
        
        if(npart <3){ //if there is only one event
            ...
            if(j == npart-2){
                ...
                inext = position_map1d_inext(proposed->p,j);// index of the next event age (younger)
                //printf("inext: %d, age of the next: %f \n",inext,proposed->models[inext].local_parameter[*li]);
                //printf("j:%d, new age: %f, younger ev:%f\n",j,proposed->models[iy].local_parameter[*li],proposed->models[inext].local_parameter[*li]);
                    if(proposed->models[iy].local_parameter[*li] > proposed->models[inext].local_parameter[*li]){
                        itest = 1;
                        //printf("chronoooo ok\n");
                            }
            }
            // youngest event case
                ...
                iprev = position_map1d_iprev(proposed->p,j);// index of the previous event age (older)
                //printf("iprev: %d, age of the prev: %f \n",iprev,proposed->models[iprev].local_parameter[*li]);
                //printf("j:%d, new age: %f, older ev:%f\n",j,proposed->models[iy].local_parameter[*li],proposed->models[iprev].local_parameter[*li]);
                if(proposed->models[iy].local_parameter[*li] < proposed->models[iprev].local_parameter[*li]){
                    itest = 1;
                    //printf("chronoooo ok\n");
                }
            }
            // other events case
                ...
                //printf("inext: %d, age of the next: %f \n",inext,proposed->models[inext].local_parameter[*li]);
                //printf("iprev: %d, age of the prev: %f \n",iprev,proposed->models[iprev].local_parameter[*li]);
                //printf("j:%d, new age: %f, younger: %f older ev:%f\n",j,proposed->models[iy].local_parameter[*li],proposed->models[inext].local_parameter[*li], proposed->models[iprev].local_parameter[*li]);
                if(proposed->models[iy].local_parameter[*li] > proposed->models[inext].local_parameter[*li]
               && proposed->models[iy].local_parameter[*li] < proposed->models[iprev].local_parameter[*li]){
                itest = 1;
                //printf("chronoooo ok\n");
                }
            }
            
        } // end of number of event
        if (proposed->models[iy].local_parameter[*li] < local_parameters[*li].fmin ||
            proposed->models[iy].local_parameter[*li] > local_parameters[*li].fmax) {
            itest = 0;
            //printf("No chrono\n");
            //printf("j:%d, new age: %f\n",j,proposed->models[iy].local_parameter[*li]);
            //return 0;
        }
    }
					 ...
					 double *value_prob)
{
  ...
  proposed->global_parameter[gi] += normal() * global_parameters[gi].fstd_value;
    ...
    i=proposed->npartitions-2;
    iy = position_map1d_give_index(proposed->p,i);
        //printf("older event: %f global:%f \n",current->models[iy].local_parameter[0],proposed->global_parameter[gi]);
        if (proposed->global_parameter[gi] < current->models[iy].local_parameter[0]) {
            return 0;
        }
    }
  ...
  move_iy = rjmcmc_random_choose_int(2, 
    ...
    while (itest==0 ) {
        ...
        if(npart < 4){
            ...
            j = position_map1d_give_index2(proposed->p,move_iy);
            //printf("j:%d npart: %d \n",npart,j);
            if(j>npart-2){printf("************* pb move j=%d\n",j);}
            //printf("new age: %f\n",proposed->models[new_iy].local_parameter[li]);
            ...
            if(j == npart-2){
                inext = position_map1d_inext(proposed->p,j);// index of the next event age (younger)
                next_x = position_map1d_position_of_index(proposed->p,inext);
                //printf("1) j: %d, inext: %d, next_x: %f new_x:%f\n",j,inext,next_x,new_x);
                //printf("1- inext: %d, age of the next: %f \n",inext,proposed->models[inext].local_parameter[li]);
                if(new_x>next_x){
                   // printf("move ok\n");
                    itest = 1;
        ...
        // youngest event case
                ...
                iprev = position_map1d_iprev(proposed->p,j);// index of the previous event age (older)
                prev_x = position_map1d_position_of_index(proposed->p,iprev);
                //printf("2) j: %d, iprev: %d, prev_x: %f, new_x:%f\n",j,iprev,prev_x,new_x);
                if(new_x<prev_x){
                    //printf("move ok\n");
                    itest = 1;
                    //return 0;
            ...
            // other events case
                ...
                //printf("3- iprev: %d, age of the prev: %f \n",iprev,proposed->models[iprev].local_parameter[li]);
                prev_x = position_map1d_position_of_index(proposed->p,iprev);
                //printf("3) j: %d, iprev: %d, prev_x: %f, new_x:%f\n",j,iprev,prev_x,new_x);
                next_x = position_map1d_position_of_index(proposed->p,inext);
                //printf("3) j: %d, inext: %d, next_x: %f, new_x:%f\n",j,inext,next_x,new_x);
                if(new_x<prev_x && new_x>next_x){
                    //printf("move ok\n");
                    itest = 1;
                    //return 0;
                }
            } // elseif
        } //end of if there is more than 1 event
            if (new_x < proposed->xmin ||
                new_x > proposed->xmax) {
                itest = 0;
            }
					 ...
					 double *hierarchical_prob)
{
  ...
  /*if ((proposed->hierarchical_parameters[hierarchicalindex] <
    ...
    return 0; // Tesson 2018: we don't want hierachical move.
